...
# Fica faltando exp(i(m-ν)ϕ₀) * exp (-iβz₀): Termos de deslocamento (z₀) e ordem (ν) do feixe
...
# Calcula um campo de Bessel centrado na origem (0, 0, 0)
function partialwavexp( 
    ψ₀::Number,
    order::Int,
    r::Float64,
    k::Float64,
    ...
    ϕ::Float64,
    nmax::Int,
    precomputed_bscs_arr::Vector{ComplexF64},
    precomputed_legendre_arr::Vector{Float64}
)

    #   A função de Bessel J_ν(0) é 1 para ν=0 e 0 para ν≠0.
    if r == 0.0 # Só ordem zero contribui no centro
        return order == 0 ? ComplexF64(ψ₀) : Complex(0.0)
    end

    kr = k * r
    ψ = ComplexF64(0.0)

    for n in abs(order):nmax
        ter
    end
