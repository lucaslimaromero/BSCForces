...
function makeplotbessel(nx, ny, rx, ry, ψ₀)
    ...
    savefig(plot(heatmaps..., layout=(3, 3), size=(1200, 900)), "plot_par.png")
end

function bessel_heatmap(x, y, nx, ny, ψ₀, αi, k, νi, z, titlechar)
    ...
    return heatmap(1000 * x, 1000 * y, bessel_beam,
        xlabel=latexstring("x (mm)"),
        ylabel=latexstring("y (mm)"),
        title=latexstring('(' * titlechar * ')'))
end

function makeplotbesseldagger(nx, ny, rx, ry, ψ₀)
    z = 0
    x = range(-rx, rx, nx)
    y = range(-ry, ry, ny)
    k = 1000 * 2 * pi / 1.54
    ...
    for αi in α
        for νi in ν
            alfcount += 1
            titlechar = alf[alfcount]
            push!(tasks, Dagger.@spawn bessel_heatmap(x, y, nx, ny, ψ₀, αi, k, νi, z, titlechar))
        end
        x = x / mag
        y = y / mag
        mag = 4
    end
    # Espera todas as tarefas terminarem e coleta os heatmaps
    heatmaps = fetch.(tasks)
    savefig(plot(heatmaps..., layout=(3, 3), size=(1200, 900)), "plot_par_dagger.png")
end

@time makeplotbessel(200, 200, 0.2, 0.2, 1)
#@time makeplotbesseldagger(200, 200, 0.2, 0.2, 1)
